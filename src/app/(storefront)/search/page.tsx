...
function SearchContent() {
  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      searchProducts(query, undefined, 1, false);
    } else {
      // Load all products when no search query
      searchProducts('', undefined, 1, false);
    }
    fetchCategories();
  ...
  }, [loading, loadingMore, hasMore, page, searchQuery, selectedCategory]);
  ...
  const searchProducts = async (query: string, categoryId?: string, pageNum: number = 1, append: boolean = false) => {
    if (append) {
      setLoadingMore(true);
    } else {
      setLoading(true);
  ...
  const loadMoreProducts = () => {
    searchProducts(searchQuery, selectedCategory, page + 1, true);
  };
  ...
  const handleCategoryFilter = (categoryId: string) => {
    ...
    searchProducts(searchQuery, categoryId, 1, false);
  };
